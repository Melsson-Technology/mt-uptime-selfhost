...
/// <summary>
/// The MT-Uptime web host, as two calls.
/// <para>
/// This is the whole server: storage, the monitoring engine, authentication, the authorization
/// policies, rate limiting, the security headers, and the endpoint map. <c>Program.cs</c> is a short
/// composition on top of it.
/// </para>
/// <para>
/// <b>Why it is a pair of extension methods rather than a Program.cs.</b> A great deal of what follows
/// is security-critical and much of it exists because of a specific review finding — the session-stamp
/// revocation in <c>OnValidatePrincipal</c>, the authenticated-by-default fallback policy, the
/// forwarded-headers trust boundary the rate limiters partition on, the Blazor circuit gate, the
/// stale-antiforgery recovery. Any second host that wanted this pipeline would otherwise copy it, and a
/// copy drifts: the next fix lands here and silently does not reach the copy, with nothing failing to
/// say so. One definition, with the genuinely host-specific choices named in
/// <see cref="MtUptimeOptions"/>, is the only arrangement where that cannot happen.
/// </para>
/// </summary>
public static class MtUptimeHosting
{
    /// <summary>
    /// Registers every service the host needs. Call before <c>builder.Build()</c>.
    /// </summary>
    public static WebApplicationBuilder AddMtUptime(
        this WebApplicationBuilder builder, Action<MtUptimeOptions>? configure = null)
    {
        var options = new MtUptimeOptions();
        ...
        var dbPath = ResolvePath(options.DatabasePath
            ?? builder.Configuration["Storage:DatabasePath"] ?? "App_Data/mt-uptime.db");
        var keysPath = ResolvePath(options.DataProtectionKeysPath
            ?? builder.Configuration["Storage:DataProtectionKeysPath"] ?? "App_Data/keys");

        // Only the database directory is created when this host owns the database. A host pointed at a
        // shared server has no local database file, and creating a directory for one would be noise.
        if (options.ConfigureDatabase is null) Directory.CreateDirectory(Path.GetDirectoryName(dbPath)!);
        ...
        // --- EF Core via a thread-safe context factory ---
        if (options.ConfigureDatabase is { } configureDatabase)
        {
        ...
        if (options.EnableFirstRunSetup)
        {
    ...
    /// <summary>
    /// Builds the middleware pipeline and maps every endpoint. Call after <c>builder.Build()</c>.
    /// <para>
    /// Asynchronous because startup may migrate the database and settle the first-run token before the
    /// first request is served — both of which must finish before anything is reachable.
    /// </para>
    /// </summary>
    public static async Task<WebApplication> UseMtUptimeAsync(this WebApplication app)
    {
        var options = app.Services.GetRequiredService<MtUptimeOptions>();

        app.UseForwardedHeaders();

        if (options.MigrateDatabaseAtStartup)
        {
        ...
        if (options.MapDatabaseAdminEndpoints)
            app.MapAdminEndpoints(options.DatabasePath!);        // already .RequireAuthorization() on both routes

        if (options.MapPushEndpoints)
