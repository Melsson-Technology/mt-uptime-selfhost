...
/// <para>
/// The case throughout is modelled on the real alert this work came from: a small business's site
/// reported DOWN with a 21,556 ms response, <c>Unexpected status 521</c>, and nothing else. Everything
/// needed to name the cause was in the response and was being discarded.
/// </para>
/// </summary>
public class OutageDiagnosticsTests
{
    ...
    private const string CloudflareBody = """
        <!DOCTYPE html><html><head><title>bakery.example | 521: Web server is down</title>
    ...
    /// </summary>
    [Fact]
    public void The_521_alert_now_names_the_cause()
    {
        var text = NotificationRenderer.PlainText(BakeryOutage(), AlertVerbosity.Rich);

        // What it always said.
        Assert.Contains("Example Bakery is DOWN.", text);
    ...
    {
        var evt = BakeryOutage() with { DetailsUrl = null };
    ...
    private static NotificationEvent BakeryOutage() =>
        new(7, "Example Bakery", MonitorStatus.Down, MonitorStatus.Up,
